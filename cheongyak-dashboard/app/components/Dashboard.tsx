...
type AnnouncementItem = Record<string, string | number>;
type CompetitionItem = Record<string, string | number>;
type StatItem = Record<string, string | number>;
...
export default function Dashboard() {
  ...
  const announcements: AnnouncementItem[] = annData?.items ?? [];
  const totalCount: number = annData?.totalCount ?? 0;
  const compItems: CompetitionItem[] = cmpData?.competition?.items ?? [];
  const statItems: StatItem[] = statData?.items ?? [];

  // 경쟁률 TOP 8: HOUSE_TY별 최고 경쟁률
  const cmpByType: Record<string, { name: string; rate: number; area: string }> = {};
  compItems.forEach((item) => {
    const key = String(item.HOUSE_MANAGE_NO ?? "");
    const rate = parseFloat(String(item.CMPET_RATE ?? "0")) || 0;
    if (!cmpByType[key] || rate > cmpByType[key].rate) {
      cmpByType[key] = {
        name: String(item.PBLANC_NO ?? key),
        rate,
        area: String(item.RESIDE_SENM ?? ""),
      };
    }
  });
  const ranked = Object.values(cmpByType)
    .sort((a, b) => b.rate - a.rate)
    .slice(0, 8);

  // 월별 통계 (지역별 신청 집계를 월별로 변환)
  ...
  statItems.forEach((item) => {
    ...
    monthMap[label] = (monthMap[label] ?? 0) +
      (Number(item.AGE_30) + Number(item.AGE_40) + Number(item.AGE_50) + Number(item.AGE_60));
  });
  ...
  const avgRate = compItems.length > 0
    ? (compItems.reduce((s, i) => s + (parseFloat(String(i.CMPET_RATE ?? "0")) || 0), 0) / compItems.length).toFixed(1)
    : "-";
  const kpis = [
    { label: "분양 공고 건수", value: totalCount.toLocaleString(), unit: "건", delta: 12.4, sub: "전체 누적", spark: [80, 90, 85, 100, 110, 120, totalCount / 100] },
    { label: "평균 경쟁률",   value: avgRate, unit: ":1", delta: 14.2, sub: "조회 기준", sparkColor: "#F97316", spark: trend.map((t) => t.rate) },
    { label: "총 공급 세대",  value: totalSupply ? totalSupply.toLocaleString() : "-", unit: "세대", delta: 6.8, sub: "조회 기준", sparkColor: "#0D9488", spark: [3000, 4000, 3500, 5000, 4800, 6000, totalSupply / 100 || 1] },
    { label: "API 연동",      value: "3", unit: "개", delta: 0, sub: "실시간 데이터", sparkColor: "#7C3AED", spark: [1, 2, 2, 3, 3, 3, 3] },
  ];
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr", gap: 18 }}>
            <Card title="청약 일정" sub="분양 공고 목록 (최신순)">
              <div style={{ display: "grid", gridTemplateColumns: "90px 1.4fr 1fr 0.8fr 0.8fr 90px", gap: 0, fontSize: 12.5 }}>
                ...
                {announcements.slice(0, 8).map((c, i) => {
                  const startDate = String(c.GNRL_RNK1_CRSPAREA_RCPTDE ?? c.RCEPT_BGNDE ?? "-");
            ...
            <Card title="공급 유형 구성" sub="주택구분 기준">
              <Donut
                data={SUPPLY_TYPES}
                centerValue={totalCount ? String(Math.min(totalCount, 9999)) : "—"}
                centerLabel="공고건"
              />
              <div style={{ marginTop: 16, padding: "12px 14px", background: "var(--bg)", borderRadius: 10 }}>
                ...
                {statItems.slice(0, 5).map((item, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, padding: "3px 0", borderBottom: i < 4 ? "1px solid var(--line)" : "none" }}>
...
function Skeleton({ h }: { h: number }) {
  return <div style={{ height: h, background: "var(--track)", borderRadius: 8, animation: "pulse 1.5s ease-in-out infinite" }} />;
}
